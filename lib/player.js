...
Player.prototype._detectPattern = function (node) {
  ...
  if (node.loc.start.line == prevNode.loc.start.line) {
    ...
    if (found) {
      console.log('PATTERN', this.factor);
      this.factor = Math.max(0.5, this.factor * 0.7);
    } else {
      console.log('DEPATTERN');
      this.factor = 1;
    }
  }
};
...
Player.prototype.play = function () {
  ...
  this.nodesWithoutSounds = {};
  console.time('step');
  this._step();
};

/**
 * Single step (play node).
 */
Player.prototype._step = function () {
  console.timeEnd('step');
  var node = this.path[this.index];
  if (!node) {
    console.log(this.nodesWithoutSounds);
    return;
  }
  ...
  if (!this.sounds.play(node)) {
    this.nodesWithoutSounds[node.type] = true;
  }
...
/**
 * Update values and DOM after `this._step`
 */
